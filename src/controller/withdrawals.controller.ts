...
export const withdrawalCallback = async (req: Request, res: Response) => {
  try {
    ...
    // 2) Parse full payload & verifikasi signature Hilogate
    const full = JSON.parse(rawBody) as any;

    // Hilogate signature pakai entire payload, bukan full.data
    const minimalPayload = JSON.stringify(full);
    const expectedSig = crypto
      .createHash('md5')
      .update('/api/v1/withdrawals' + minimalPayload + config.api.hilogate.secretKey, 'utf8')
      .digest('hex');
    ...
    // 3) Extract langsung dari root full
    const { ref_id, status, net_amount, completed_at, amount, total_fee } = full;
    ...
    // 6) Update WithdrawRequest (termasuk total_fee jika ada di schema)
